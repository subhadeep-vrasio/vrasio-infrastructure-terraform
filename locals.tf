...
locals {
  web_user_data = <<-EOF
    ...
    # -------------------------------
    # Apache vhost for dev.vrasio.com
    # -------------------------------
    cat <<'VHOST' > /etc/httpd/conf.d/dev.conf
    <VirtualHost *:80>
        ServerName dev.vrasio.com
        ...
        CustomLog /var/log/httpd/dev_access.log combined
    </VirtualHost>
    VHOST
    ...
    # -------------------------------
    printf "\n" | pecl install imagick >> $LOG 2>&1
    echo "extension=imagick.so" | tee /etc/php.d/20-imagick.ini >> $LOG 2>&1
    ...
    # Redis from source
    # -------------------------------
    dnf install -y gcc make jemalloc-devel wget >> $LOG 2>&1
    cd /usr/local/src
    wget http://download.redis.io/redis-stable.tar.gz >> $LOG 2>&1
    tar xzvf redis-stable.tar.gz >> $LOG 2>&1
    cd redis-stable
    ...
    # Create redis user
    id redis >/dev/null 2>&1 || useradd -r -s /bin/false redis
    ...
    # Bind only localhost + supervised systemd
    ...
    echo "Checking Redis status..." >> $LOG
    systemctl status redis >> $LOG 2>&1 || echo "Redis failed to start" >> $LOG
